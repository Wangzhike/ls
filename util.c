...
int comp_finfos_S(const void *a, const void *b)
{
	...
	/* qsort sorted array in ascending order, but `-S` sorted with the largest first */
	return d - c;
}
...
void set_c_size(columns_t *col, int nlink, char *user, char *group, long long int size)
{
	int len;
...
{
	 char *s1 = (char *)(**dirent1).d_name;
	 char *s2 = (char *)(**dirent2).d_name;
